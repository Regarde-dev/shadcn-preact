/**
 * Preact Chat State Management
 * 
 * Implements ChatState interface for Preact using subscription pattern.
 * Based on @ai-sdk/react Chat class but adapted for Preact.
 */
...
/**
 * Preact-specific chat state implementation
 * Manages messages, status, and error state with subscription callbacks
 */
class PreactChatState<UI_MESSAGE extends UIMessage> implements ChatState<UI_MESSAGE> {
  #messages: UI_MESSAGE[];
  ...
  replaceMessage = (index: number, message: UI_MESSAGE) => {
    this.#messages = [
      ...
      // Deep clone to ensure nested changes are detected
...
/**
 * Preact Chat class
 * Extends AbstractChat with Preact-specific state management
 */
export class Chat<UI_MESSAGE extends UIMessage> extends AbstractChat<UI_MESSAGE> {
  #state: PreactChatState<UI_MESSAGE>;

  constructor({ messages, ...init }: ChatInit<UI_MESSAGE>) {
    const state = new PreactChatState(messages);
    super({ ...init, state });
    this.#state = state;
  }
